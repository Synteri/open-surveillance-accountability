...
1. **What evidentiary posture does the record support?** `Verified`, `Vendor-asserted`, `Partially verifiable`, and `Unknown` describe how well the underlying fact is established. `Noncompliant` is reserved for a claim-matrix finding that cited evidence establishes failure against an identified binding obligation; it is not a confidence level.
...
Fairfield inventory rows use the four factual evidence postures—`Verified`, `Vendor-asserted`, `Partially verifiable`, and `Unknown`—plus the shared implementation-state vocabulary. They deliberately omit an OASPS assessment and cannot use `Noncompliant`; they identify systems and current evidence posture rather than making abbreviated compliance findings.

## Review status

This draft has undergone owner-directed internal adversarial and AI-assisted review. That work is quality control, not independent third-party assurance. The project has not completed independent third-party privacy, public-law, or software-assurance review.

Historical comments and reviews on [PR #1](https://github.com/Synteri/open-surveillance-accountability/pull/1) that used the phrase “independent review” were submitted through the repository owner's account. They document owner-controlled review, not outside endorsement. Automated validation checks repository structure and declared invariants; it does not establish factual completeness, legal validity, production behavior, security effectiveness, or approval by an outside institution.
